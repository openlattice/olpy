...
class EntityDefinition(object):
    ...
    def entity_set_validation(self, all_entity_sets=[], already_done=dict(), log_level="none"):
        ...
        try:
            existing_entity_set = self.entity_sets_api.get_entity_set(
                self.entity_sets_api.get_entity_set_id(self.entity_set_name))
            if self.get_entity_type().id != existing_entity_set.entity_type_id:
                report.validated = False
                report.issues.append(
                    f"The fqn listed for {self.entity_set_name} does not match that of the existing entity set.")
...
class Flight(object):
    ...
    def transform_schema(self, type="fqn"):
        ...
        for association in self.schema['associationDefinitions'].values():
            ...
            if not (len(entdsts) == 1 and len(entsrcs) == 1):
                raise ValueError(
                    "  - The source and destination for association \n    %s are not (uniquely) defined." % association[
                        'name'])
    ...
    def check_entsets_against_stack(self, create_by=[]):
        ...
        for entity in list(self.entity_definitions.values()) + list(self.association_definitions.values()):
            ...
            try:
                prod_entset = self.entity_sets_api.get_entity_set(
                    self.entity_sets_api.get_entity_set_id(entity.entity_set_name))
                if entity.get_entity_type().id != prod_entset.entity_type_id:
                    sub_reports['fqn_match'].issues += [
                        f"The entity type of {entity.entity_set_name} does not match the existing entity set."]
            except:
                if len(create_by) > 0:
                    try:
    ...
    def proofread(self, table_columns=None, authenticated=True, create_by=[]):
        ...
        src_dst = set([x.src_alias for x in self.association_definitions.values()] + [x.dst_alias for x in
                                                                                      self.association_definitions.values()])
        ...
        if len(missing_entities) > 0:
            print("The following entity sets are not correctly referenced in their association: %s" % ", ".join(
                missing_entities))
        ...
        for assoc_def in self.association_definitions.values():
            ...
            src_def = self.entity_definitions[
                assoc_def.src_alias] if assoc_def.src_alias in self.entity_definitions else [x for x in
                                                                                             self.entity_definitions.values()
                                                                                             if
                                                                                             x.name and x.name == assoc_def.src_alias]
            dst_def = self.entity_definitions[
                assoc_def.dst_alias] if assoc_def.dst_alias in self.entity_definitions else [x for x in
                                                                                             self.entity_definitions.values()
                                                                                             if
                                                                                             x.name and x.name == assoc_def.dst_alias]
    ...
    def validate_flight_against_atlas(self, table_name=None, engine=None, log_level="none"):
        ...
        if len(missing_columns) > 0:
            ...
            missing_columns_report.issues.append(
                "The following columns are missing from the table: %s\n" % ", ".join(missing_columns))
    ...
    def source_destination_validation(self, log_level="none"):
        ...
        report = clean.report.ValidationReport(
            ...
            issues=["Consider calling flight.add_src_dst_to_edm() with the current flight instance."]
            # issues are only shown if validated == False
        )
        ...
        for key, defn in list(self.entity_definitions.items()) + list(self.association_definitions.items()):
            for alias, prop in defn.property_definitions.items():
                ...
                if re.search("Date[a-zA-Z]*Transform", tf) and "timezone" in tf:
                    report.validated = False
                    report.issues.append(
                        f"In {key}, {alias} is using a Date*Transform with timezone argument. This is deprecated and will not have the desired behavior.")
        report.validate()
        report.print_status(log_level=log_level)
        return report
    ...
    def graph_connectivity_validation(self, log_level="none"):
        ...
        if len(missing_associations) > 0:
            report.validated = False
            report.issues.append(
                "The following entity sets are not linked to anything: %s" % ", ".join(missing_associations))
    ...
    def add_src_dst_to_edm(self):
        for assoc_def in self.association_definitions.values():
            ...
            if assoc_type:
                if src_def.get_entity_type().id not in assoc_type.src:
                    self.edm_api.add_src_entity_type_to_association_type(assoc_type.entity_type.id,
                                                                         src_def.get_entity_type().id)
                    print(assoc_def.fqn, src_def.fqn)
                if dst_def.get_entity_type().id not in assoc_type.dst:
                    self.edm_api.add_dst_entity_type_to_association_type(assoc_type.entity_type.id,
                                                                         dst_def.get_entity_type().id)
    ...
    def fill_in(
        ...
        for alias, entity in list(self.entity_definitions.items()) + list(self.association_definitions.items()):
            ...
            if not entity.entity_set_name and create_entity_set_names:
                entity.entity_set_name = jurisdiction_string + "".join(
                    [re.sub("[0-9]+", "", word.capitalize()) for word in name_words])
        ...
        if gen_missing_pks:
            for entity in self.entity_definitions.values():
                entity.add_pk_if_missing(infer_columns=infer_pk_cols, columns=default_pk_cols,
                                         hash_by_default=hash_by_default)
...
def _add_missing_association_pks(flight, edmAPI, infer_columns=False, columns=[], hash_by_default=False):
    for entname, entity in flight["associationDefinitions"].items():
        if infer_columns:
            columns = list(clean.utils.get_cols_from_pk(flight["entityDefinitions"][entity["src"]],
                                                        edmAPI) | clean.utils.get_cols_from_pk(
                flight["entityDefinitions"][entity["dst"]], edmAPI))
        keyprops = ["ol.id"]
        ...
        if len(overlap) == 0:
            if not hash_by_default:
                if len(columns) == 0:
                    ...
                    entity['propertyDefinitions'][keyprops[0]] = {"type": keyprops[0], "transforms": [
                        {"transforms.ConcatTransform": None, "columns": columns}]}
            else:
                entity['propertyDefinitions'][keyprops[0]] = {"type": keyprops[0], "transforms": [
                    {"transforms.HashTransform": None, "columns": columns, "hashFunction": "sha256"}]}
...
def deduce_edm_object(name, edm_api=None, category="entity", from_pool=None):
    ...
    search_space = {x.type.namespace: x.type.name for x in from_pool if
                    x.type.name.lower() == name.lower() and "-d" not in x.type.name.lower() and "-d" not in x.description.lower() and "-d" not in x.title.lower()}
